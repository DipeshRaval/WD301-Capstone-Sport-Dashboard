...
export default function Match(props: Props, State: State) {
  ...
  const handleLike = (id : number) => {
    if(isLoggedIn)
    {
      ...
      const likematch = localStorage.getItem("guestLikeMatches")
      console.log(typeof(likematch));
      
      if(likematch){
        const updateLikes = JSON.parse(likematch).append(id)
        console.log(updateLikes);
        localStorage.setItem("guestLikeMatches", JSON.stringify(updateLikes))

      }else{
        localStorage.setItem("guestLikeMatches", JSON.stringify([id]))
      }
      ...
      {match.isRunning && (
        <div className="border-2 mx-2 mb-1 rounded border-gray-400 bg-gray-100 dark:bg-slate-600 p-2">
          <div className=" flex justify-between w-48">
            ...
            <div className="flex">
              <button className="mx-2" onClick={()=> (handleLike(match.id))}>
                {localStorage.getItem("guestLikeMatches")?.includes(`${match.id}`) ? (<svg
