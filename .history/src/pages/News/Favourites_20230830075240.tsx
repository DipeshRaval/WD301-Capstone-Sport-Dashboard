...
export default function Favourites() {
  ...
  const settingOptionState = async () => {
    if(isLoggedIn){
      ...
      if(Object.keys(data.preferences).length)
      {
        ...
        data.preferences.SelectedTeams.length !== 0 ? setOptionFevTeam(data.preferences.SelectedTeams) : setOptionFevTeam(teams.map((team:Team) => team.name))
        
        if(data.preferences.SelectedTeams.length !== 0){
          setOptionFevTeam(data.preferences.SelectedTeams)
        }else{
          if (fevSport && fevSport !== "Favourite Sport") {
            // setOptionFevTeam(teams.filter((team: Team) => {
            //   if(team.plays === fevSport)
            //     return team.name
            // }))

            console.log(teams.filter((team: Team) => {
                if(team.plays === fevSport)
                  return team.name
              }))
          }else{
      ...
      if (fevSport && fevSport !== "Favourite Sport") {
        ...
        console.log(optionFevTeam.filter((team: Team) => {
          if(team.plays === fevSport)
          {
            return (team.name)
          }
  ...
  console.log(optionFevSport);
  

  useEffect(()=>{
    settingOptionState()
  }, [isOpen, fevSport, isLoading])
