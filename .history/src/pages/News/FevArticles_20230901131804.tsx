  ...
  const settingFevArticles = async () => {
    let intialNews = [...news];
      ...
      if (fevSport && fevSport !== "Favourite Sport") {
        intialNews = intialNews.filter((newsItem: any) => {
    ...
    if (isLoggedIn) {
      ...
      if (data && Object.keys(data.preferences).length) {
        if (fevSport !== "Favourite Sport" || fevTeam !== "Favourite Team" && data.SelectedSport.length || data.SelectedTeams.length) {
          if(fevSport !== "Favourite Sport")
          {
            ...
            //       preferanceArticles.push(newsItem)
            //     }
            //   });
            // }
          }
        } else {
          let preferanceArticles: News[] = [];
          ...
          //   news.forEach((newsItem: any) => {
          //     if (data.preferences.SelectedTeams.includes(newsItem.sport.name)) {
          //       preferanceArticles.push(newsItem)
          //     }
          //   });
          // }

          setFevArticles(preferanceArticles);
        }
      } else {
        setFevArticles(news);
      }
